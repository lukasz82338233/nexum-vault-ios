#!/bin/bash
# verify-build-ready.sh - check required files before opening Xcode or running CI.
...
if [ "$ERRORS" -eq 0 ]; then
  ...
  echo "Open NexumVault.xcodeproj in Xcode, set your team, then build to device."
else
